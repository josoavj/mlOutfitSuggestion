...
class PreferencesStore:
    ...
    def apply_micro_survey_response(self, user_id: str, response: MicroSurveyResponse) -> UserPreferences:
        ...
        if response.accepted:
            ...
            # formality_check / color_check : l'ajustement précis dépend de la
            # question posée par l'appelant — à affiner selon le libellé exact
            # du micro-questionnaire déclenché (voir preferences/triggers.py).

        prefs.last_micro_survey_at = datetime.utcnow()
        prefs.preferences_version += 1
        self._save(prefs)
        return prefs
